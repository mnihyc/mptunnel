...
impl ResponseStreamBinding {
    ...
    fn commit_reinjected_frame_with_shape(
        ...
        // freeze the Latency objective through the early-copy transaction.
        let current_lane = early_completion_copy
            .then(|| self.lane.lock().expect("server reliable stream lane lock"));
        if early_completion_copy
            && (lane != TrafficClass::Latency
                || current_lane
                    .as_ref()
                    .is_some_and(|lane| **lane != TrafficClass::Latency))
        {
            return Err(RuntimeError::SenderServiceBlocked);
        }
        let mut outputs = self
        ...
        if early_completion_copy {
            let frontier = reliable_live_owner_uniform_frontier(
                ...
                flights.range(..end).flat_map(|(start, entries)| {
                    entries.iter().filter_map(|flight| {
                        let identity = ServerReinjectionOutputIdentity {
                            key: flight.key,
                            incarnation: flight.output_incarnation,
                        };
    ...
    /// First uncopied prefix only; raw accepted debt survives expiry and detach.
    pub(in crate::runtime) fn uncopied_completion_prefix(
        &self,
        range: OffsetRange,
    ) -> Option<OffsetRange> {
        let flights = self
            ...
            .lock()
            .expect("server reliable stream flight lock");
        uncopied_completion_prefix(&flights, range)
    }
