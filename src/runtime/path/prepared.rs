...
/// Native reads happen before final Product ownership. Only the exact FIFO's
/// operation metadata is consulted with the current Product frontier afterward.
...
impl<K: Copy + Eq> PreparedNativeCommitmentInputs<K> {
    ...
    /// This uses the captured Native result and current Product frontier; it
    /// never queries Native while the final Product owner is held.
    pub(in crate::runtime) fn check_selected(
        ...
        let view = captured.as_ref().map_err(Clone::clone)?;
        view.barrier(current_frontier).map(|_| ())
    }
