//! Linux implementation of the platform-neutral VPN generation boundary.

use super::super::linux_vpn::{
    LinuxVpnShutdownError, PreparedLinuxVpn, compile_linux_vpn_prepare_request, prepare_linux_vpn,
};
...
const PLATFORM: VpnPlatform = VpnPlatform::Linux;

pub(super) fn validate(config: &AppConfig) -> Result<(), VpnGenerationError> {
    compile_linux_vpn_prepare_request(config)
        .map(|_| ())
...
pub(super) async fn prepare(
    ...
    let request = compile_linux_vpn_prepare_request(config).map_err(|error| {
        VpnGenerationError::adapter(PLATFORM, VpnGenerationStage::Validate, error)
    })?;
    match request {
        Some(request) => prepare_linux_vpn(request)
            .await
...
pub(super) struct PreparedVpnGeneration(PreparedLinuxVpn);
        ...
        self.0
            ...
            .map_err(|error| {
                let stage = match &error {
                    LinuxVpnShutdownError::PublicationStillActive { .. }
                    | LinuxVpnShutdownError::Unpublish(_) => VpnGenerationStage::Unpublish,
                    LinuxVpnShutdownError::PacketWorkerStillRunning => VpnGenerationStage::Stop,
                    LinuxVpnShutdownError::Cleanup(_) => VpnGenerationStage::Cleanup,
                };
