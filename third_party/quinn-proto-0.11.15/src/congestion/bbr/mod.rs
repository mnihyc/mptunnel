...
use crate::congestion::{ControllerMetrics, PacketDeliveryState};
...
impl Controller for Bbr {
    ...
    fn on_ack(
        ...
        self.record_ack(now, sent, bytes, app_limited, None, rtt);
    }
    ...
    fn on_ack_with_packet_state(
        ...
        self.record_ack(now, sent, bytes, app_limited, packet_state, rtt);
    }
    ...
    fn on_end_acks(
        ...
        let sample_app_limited = self.max_bandwidth.end_acks(self.round_count, self.min_rtt);
        ...
        if is_round_start && !self.is_at_full_bandwidth {
            if let Some(sample_app_limited) = sample_app_limited {
...
impl Bbr {
    fn record_ack(
        ...
        if let Some(packet_state) = packet_state {
            self.max_bandwidth.on_ack(now, sent, bytes, packet_state);
        } else {
            self.max_bandwidth.on_ack_without_packet_state(now, bytes);
        }
        self.acked_bytes += bytes;
        let first_rtt_sample = self.probe_rtt_last_started_at.is_none();
        if first_rtt_sample || self.is_min_rtt_expired(now, app_limited) || self.min_rtt > rtt.min()
        {
