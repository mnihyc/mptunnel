...
#[test]
fn raw_completion_prefix_survives_expiry_detachment_and_ack_splits() {
    let owner = path(UnderlayProtocol::Udp, 0, 401);
    let copy = path(UnderlayProtocol::Tcp, 1, 402);
    let mut ledger = RequestFlightLedger::default();
    let full = OffsetRange { start: 0, end: 24 };
    ledger.record_original_frame_instance(owner, &data_frame(0, 24));
    assert_eq!(ledger.raw_uncopied_prefix(full), Some(full));
    let (_, deadline) = ledger.record_reinjection_frame_instance_with_suppression_interval(
        copy,
        &data_frame(4, 12),
        Duration::ZERO,
    );
    // The current copy mask is empty both after expiry and after detachment;
    // neither transition renews a byte's earlier completion opportunity.
    assert_eq!(
        ledger
            .live_copy_coverage(&[owner, copy], deadline.unwrap())
            .0,
        vec![]
    );
    assert_eq!(
        ledger.live_copy_coverage(&[owner], Instant::now()).0,
        vec![]
    );
    assert_eq!(
        ledger.raw_uncopied_prefix(full),
        Some(OffsetRange { start: 0, end: 4 })
    );
    assert_eq!(
        ledger.raw_uncopied_prefix(OffsetRange { start: 4, end: 24 }),
        None
    );
    assert_eq!(
        ledger.raw_uncopied_prefix(OffsetRange { start: 9, end: 24 }),
        None
    );
    assert_eq!(
        ledger.raw_uncopied_prefix(OffsetRange { start: 16, end: 24 }),
        Some(OffsetRange { start: 16, end: 24 }),
    );

    ledger.release_normalized_acked_ranges(&[OffsetRange { start: 8, end: 12 }]);
    assert_eq!(
        ledger.raw_uncopied_prefix(OffsetRange { start: 4, end: 8 }),
        None
    );
    assert_eq!(
        ledger.raw_uncopied_prefix(OffsetRange { start: 12, end: 24 }),
        None
    );
    assert!(
        ledger
            .live_owner_uniform_frontier(OffsetRange { start: 8, end: 12 }, &[owner])
            .is_none()
    );
    ledger.release_normalized_acked_ranges(&[OffsetRange { start: 0, end: 16 }]);
    assert_eq!(ledger.raw_uncopied_prefix(full), Some(full));
    assert!(
        ledger
            .live_owner_uniform_frontier(OffsetRange { start: 0, end: 16 }, &[owner])
            .is_none()
    );
    assert_eq!(ledger.reinjected_data_in_flight_bytes(copy), 0);
    assert_eq!(ledger.original_data_in_flight_bytes(owner), 8);
    assert_eq!(
        ledger.raw_uncopied_prefix(OffsetRange { start: 24, end: 24 }),
        None
    );
}
