...
fn request_quic_capacity_geometry_models_the_competing_service_rate_pipe() {
    ...
    candidate.carrier_inflight_limit_bytes = 262_144;
...
fn request_quic_capacity_geometry_excludes_candidate_product_flight() {
    ...
    let mut candidate = PathSnapshot::new(PathId(1), UnderlayProtocol::Udp, 180.0, 1_000_000.0);
    candidate.carrier_inflight_limit_bytes = 262_144;
    candidate.data_level_bytes_in_flight = 3_000_000;
    candidate.bytes_in_flight = 500_000;

    let first =
        request_quic_capacity_measurement_geometry(candidate, 1_000_000.0, mux_limits, envelope)
    ...
    candidate.data_level_bytes_in_flight = 7_000_000;
    let more_product =
        request_quic_capacity_measurement_geometry(candidate, 1_000_000.0, mux_limits, envelope)
            .expect("product debt must not alter carrier geometry");
    assert_eq!(more_product, first);

    candidate.bytes_in_flight = 750_000;
    let more_carrier =
        request_quic_capacity_measurement_geometry(candidate, 1_000_000.0, mux_limits, envelope)
...
fn request_quic_capacity_geometry_requires_valid_rate_and_budget() {
    ...
    assert!(
        request_quic_capacity_measurement_geometry(
            candidate,
            f64::NAN,
            mux_limits,
            reliable_capacity_measurement_session_limit_bytes(mux_limits),
    ...
    let bounded = request_quic_capacity_measurement_geometry(
        candidate,
        2_000_000_000.0,
        mux_limits,
        reliable_capacity_measurement_session_limit_bytes(mux_limits),
    ...
    carrier_loaded.bytes_in_flight = reliable_capacity_measurement_session_limit_bytes(mux_limits);
    assert!(
        request_quic_capacity_measurement_geometry(
            carrier_loaded,
            100_000_000.0,
...
fn request_quic_capacity_lease_covers_cold_congestion_window_growth() {
    ...
    let pto = transport_pto_from_snapshot(Some(candidate));
    let modeled_round_trip = Duration::from_millis(180).max(pto.div_f64(RELIABLE_PIPE_WINDOW_BDPS));

    assert_eq!(rounds, 10);
    assert!(
        request_quic_capacity_measurement_lease(candidate, train_bytes)
            >= modeled_round_trip
