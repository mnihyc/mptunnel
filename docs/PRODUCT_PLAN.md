...
| --- | --- | --- | --- |
| MPP | Yes | Yes | Multiple TCP/QUIC carrier endpoints under one logical session, one credential, exact TLS leaf pin, path policy, Core repair envelope, and an optional inclusive outbound carrier port range. TCP selects per new carrier; QUIC also migrates the same authenticated connection periodically within the set. |
...
- ECH, REALITY, periodic TCP carrier replacement, pluggable transports, or a claim of
