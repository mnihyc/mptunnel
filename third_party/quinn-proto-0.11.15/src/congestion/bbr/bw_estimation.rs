...
impl BandwidthEstimation {
    pub(crate) fn on_packet_sent(
        ...
        PacketDeliveryState {
            ...
            packet_number,
            app_limited,
            tx_in_flight: prior_in_flight.saturating_add(u64::from(bytes)),
        }
    }
    ...
    pub(crate) fn on_ack(
        ...
        let newest = self.pending_sample.is_none()
            ...
            || (sent == first_sent_time
                && self
                    .pending_sample
                    .is_some_and(|sample| packet_state.packet_number > sample.packet_number));
        if !newest {
            return;
        }
        ...
        self.pending_sample = Some(PendingRateSample {
            ...
            send_elapsed: sent.saturating_duration_since(packet_state.first_sent_time),
            ack_elapsed: now.saturating_duration_since(packet_state.delivered_time),
            packet_number: packet_state.packet_number,
            app_limited: packet_state.app_limited,
        });
