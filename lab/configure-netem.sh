...
change_scheduled_profile_observed() {
  local profile="${MPTUNNEL_LAB_SCHEDULE_PROFILE:-balanced}"
  local subnet_prefix rate delay jitter loss
  local iface address apply_exit readback_exit readback_output root_line evidence

  case "$profile" in
    lowlat)
      subnet_prefix="172.31.10"
      rate="$lowlat_rate"
      delay="$lowlat_delay"
      jitter="$lowlat_jitter"
      loss="$lowlat_loss"
      ;;
    balanced)
      subnet_prefix="172.31.15"
      rate="$balanced_rate"
  ...
  iface="$(interface_for_subnet "$subnet_prefix" || true)"
  if [[ -z "$iface" ]]; then
    printf '69\t69\t-\n'
    return 69
  fi
  ...
  address="$(
    ip -o -4 addr show dev "$iface" scope global \
      | awk -v prefix="$subnet_prefix" '$4 ~ "^" prefix "[.]" {print $4; exit}' \
      || true
  )"
  root_line="$(
    printf '%s\n' "$readback_output" \
...
profile = os.environ.get("MPTUNNEL_LAB_SCHEDULE_PROFILE", "balanced")
rate_values = os.environ.get("MPTUNNEL_LAB_SCHEDULE_RATES", "")
rates = rate_values.split(",") if rate_values else [None] * len(losses)
netem_mode = (
    "change-balanced-observed"
    if profile == "balanced" and not rate_values
    else "change-scheduled-profile-observed"
)
...
if (
    epoch_ms <= 0
    or duration_ms != epoch_ms * len(losses)
    or lateness_ms < 0
    or profile not in {"lowlat", "balanced", "fat"}
    or len(rates) != len(losses)
    or any(rate == "" for rate in rates)
):
    ...
    if exit_code == 0:
        ...
        if len(anchor_lines) != 3:
            ...
            for index, loss in enumerate(losses):
                ...
                env[f"MPTUNNEL_LAB_{profile_prefix}_LOSS"] = f"{loss}%"
                if rates[index] is not None:
                    env[f"MPTUNNEL_LAB_{profile_prefix}_RATE"] = rates[index]
                command_exit = 0
                output = ""
                try:
                    active_process = subprocess.Popen(
                        [netem_script, netem_mode],
                        env=env,
                        text=True,
...
case "$mode" in
  ...
  change-balanced-observed)
    ...
    change_balanced_observed
    ;;
  change-scheduled-profile-observed)
    # The split TCP/QUIC recovery case applies and observes one isolated path.
    change_scheduled_profile_observed
    ;;
