...
impl ServerResponseSenderService {
    ...
    pub(in crate::runtime) fn commit_prepared_recovery(
        ...
        #[cfg(feature = "lab-diagnostics")]
        let accept_begin_unix_ms =
            lab_diagnostic_event_enabled("prepared_recovery_accept").then(|| {
                std::time::SystemTime::now()
                    .duration_since(std::time::UNIX_EPOCH)
                    .expect("diagnostic wall clock after Unix epoch")
                    .as_millis()
            });
        let deadline = binding.try_enqueue_prepared_recovery_with_shape(
            &candidate.target,
            &candidate.frame,
            candidate.lane,
            self.queue
        ...
        #[cfg(feature = "lab-diagnostics")]
        if lab_diagnostic_event_enabled("prepared_recovery_accept") {
            // Observation after actual carrier queue/Product flight acceptance.
            // The final query supplied this cause; no new authority is introduced.
            lab_diagnostic(
                "prepared_recovery_accept",
                format_args!(
                    "session_id={} stream_id={} offset={} end={} payload_bytes={} cause={} proven={} frontier={} target_underlay={:?} target_path_id={} target_instance={} target_incarnation={} accept_begin_unix_ms={}",
                    self.session_id.0,
                    self.stream_id.0,
                    start,
                    end,
                    ...
                    accept_begin_unix_ms.expect("enabled acceptance observation"),
                ),
            );
        }
        Ok(deadline)
