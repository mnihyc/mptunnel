...
Encrypted UDP-underlay datagram framing is implemented at the transport layer, and the runtime has a scoped encrypted UDP datagram-flow path for direct UDP targets. SOCKS5 UDP ASSOCIATE ingress is implemented as a local association relay over encrypted UDP paths to direct UDP targets; each datagram still uses a scoped internal UDP path session until persistent UDP sessions are added. Upstream SOCKS5 UDP ASSOCIATE, HTTP CONNECT-UDP, multipath scheduling execution, and TUN-L4 runtime are still under active development.
