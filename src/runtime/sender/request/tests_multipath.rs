...
use crate::runtime::stream::{ReliableRelayRemoteInput, ReliableRelayRemoteSet};
...
struct PreparedCompletionFixture {
    context: ClientPathContext,
    remotes: ReliableRelayRemoteSet,
    _input: ReliableRelayRemoteInput,
    sender: super::super::RequestSenderService,
    send_stream: ReliableSendStream,
    queue: ReliableRelaySenderQueue,
    ack: crate::runtime::relay::io::AuthoritativeStreamAckSnapshot,
    owner: RelayPathInstance,
    target: RelayPathInstance,
    owner_receivers: crate::runtime::path::commands::ReliablePathCommandReceivers,
    target_receivers: crate::runtime::path::commands::ReliablePathCommandReceivers,
}

impl PreparedCompletionFixture {
    fn finish_attachment_proof(
        receivers: &mut crate::runtime::path::commands::ReliablePathCommandReceivers,
        expected_path: PathId,
    ) {
        let command = try_recv_reliable_path_command(receivers).expect("attachment proof");
        assert!(matches!(&command,
            ReliablePathCommand::SendFrame(Frame::PathProofData { path_id, payload, .. })
                if *path_id == expected_path && !payload.is_empty()
        ));
    ...
    fn with_initial_credit(
        ...
        Self::finish_attachment_proof(&mut owner_receivers, PathId(0));
        ...
        sender
            .send_frame(
                ...
                Some(TrafficClass::Latency),
            )
            .expect("real Original queue Apply establishes the owner before any alternate exists");
        // Installing the exact owner advances its proof generation. Ordinary
        // planning refreshes the attachment proof before publishing Product
        // data; consume that real control work and its writer charge as well.
        loop {
            let command = try_recv_reliable_path_command(&mut owner_receivers)
                .expect("successful Original Apply publishes its frame");
            let is_original = match &command {
                ...
                ReliablePathCommand::SendFrame(frame) if *frame == original => true,
                _ => panic!("expected current attachment proof or committed Original"),
            };
            owner_receivers
        ...
        let (commands, mut target_receivers) = reliable_path_command_channels(8);
        remotes.attach_candidate(opened_test_relay_stream(stream_id, 1, commands));
        Self::finish_attachment_proof(&mut target_receivers, PathId(1));
        let target = remotes.paths[1].instance();
        context.install_relay_path_instance_for_test(target);
        context.mark_tcp_path_open_success(1, Duration::from_millis(20), TrafficClass::Latency);
        Self {
            context,
            remotes,
            _input: input,
    ...
    fn query(
        ...
        now: Instant,
    ) -> super::super::RequestPreparedRecoveryObservation {
        let inputs = RequestRelayNativeCapture::new(
            self.remotes.membership_generation(),
            &self.remotes.paths,
        )
        .resolve();
        self.sender.next_prepared_recovery(
            &self.context,
            &self.remotes,
            &self.send_stream,
            &self.queue,
    ...
    fn ack(&mut self, scope_start: Option<u64>, ranges: Vec<OffsetRange>) {
        let ack = crate::runtime::relay::io::begin_reliable_stream_ack(
            &self.send_stream,
            scope_start,
            ranges,
        )
        .unwrap();
        self.sender
            .apply_request_product_ack(&self.context, &self.remotes, &mut self.send_stream, &ack)
            .unwrap();
        crate::runtime::relay::io::update_reinjection_authoritative_ack_snapshot(
            &mut self.ack,
            &ack,
            &self.send_stream,
        );
...
async fn prepared_latency_completion_uses_real_ready_apply_once_and_refusal_spends_nothing() {
    ...
    assert!(
        !f.context
            .relay_path_instance_has_bulk_model_evidence(target)
    );
    let ordinary = f.query(TrafficClass::Throughput, &[target], now);
    ...
    assert!(f.query(TrafficClass::Latency, &[], now).candidate.is_none());
    assert!(
        f.query(TrafficClass::Latency, &[f.owner], now)
            .candidate
            .is_none()
    );
    let candidate = f
        .query(TrafficClass::Latency, &[target], now)
        .candidate
        .expect("fresh retained tail has one distinct Ready completion opportunity");
    assert!(candidate.early_completion_copy);
    assert!(matches!(
        candidate.cause,
        RelaySendCause::CompletionTailReinjection(_)
    ));
    assert!(f.ack.gaps().is_empty());
    let (start, end, bytes) = reliable_stream_frame_extent(&candidate.frame).unwrap();
    ...
    assert_eq!(f.sender.multipath.raw_uncopied_prefix(range), Some(range));
    assert_eq!(f.sender.multipath.accepted_reinjected_data_bytes(target), 0);
    ...
    f.sender
        ...
        .expect("exact target Ready accepts normal carrier repair work");
    assert!(!ready.receipt().is_current());
    assert_eq!(
        f.sender.multipath.accepted_reinjected_data_bytes(target),
        bytes
    );
    assert_eq!(f.sender.multipath.raw_uncopied_prefix(range), None);
    assert_eq!(
        f.sender.optional_reinjection.reinjected_bytes(),
        bytes as u64
    );
    let command = try_recv_reliable_path_command(&mut f.target_receivers).unwrap();
    assert!(matches!(&command, ReliablePathCommand::SendFrame(frame) if *frame == candidate.frame));
    f.target_receivers
        .release_pending_command_bytes(reliable_path_command_pending_bytes(&command));
    let ready = f
        .target_receivers
    ...
    assert!(
        f.query(TrafficClass::Latency, &[target], now)
            .candidate
            .is_none()
    );
    assert_eq!(f.send_stream.next_offset(), 4096);
    assert_eq!(f.send_stream.reinjection_bytes(), 4096);
    assert!(
        !f.context
            .relay_path_instance_has_bulk_model_evidence(target),
        "completion admission leaves the target's startup prior unqualified",
    );
}

#[tokio::test]
async fn prepared_latency_completion_clips_raw_history_and_preserves_spent_head_fallback() {
    let mut f = PreparedCompletionFixture::new();
    let now = Instant::now();
    let target = f.target;
    let retired = RelayPathInstance {
        key: target.key,
        path_instance_id: next_carrier_path_instance_id(),
        attachment_id: target.attachment_id + 1,
    };
    let copied = data_frame(f.remotes.stream_id(), 1024, 1024);
    f.sender
        .multipath
        .request
        .flights
    ...
    assert!(!f.remotes.contains_path_instance(retired));
    let candidate = f
        .query(TrafficClass::Latency, &[target], now)
        .candidate
        .unwrap();
    ...
    let spent = f.query(TrafficClass::Latency, &[target], now);
    assert!(
        spent.candidate.is_none(),
        "ACK splitting and detach cannot reopen the spent head"
    );
    let fallback = spent
        .next_deadline
        .expect("ordinary fallback is still armed");
    assert!(fallback > now);
    assert!(
        f.query(TrafficClass::Latency, &[target], fallback)
            .candidate
            .is_none(),
        "maturity alone does not relax the unchanged normal prepared target policy",
    );
    f.ack(
        None,
        vec![OffsetRange {
    ...
    let suffix = f
        ...
        .candidate
        .unwrap();
    assert!(suffix.early_completion_copy);
    assert_eq!(
        reliable_stream_frame_extent(&suffix.frame),
        Some((2048, 4096, 2048))
    );
}

#[tokio::test]
async fn prepared_latency_proven_completion_rechecks_raw_acceptance_at_apply() {
    let mut f = PreparedCompletionFixture::new();
    let target = f.target;
    assert!(
        !f.context
    ...
    // A separately accepted copy can expire and retire between advisory Decide
    // and final Apply. Its raw overlap still defeats this stale opportunity.
    let retired = RelayPathInstance {
        key: target.key,
        path_instance_id: next_carrier_path_instance_id(),
        attachment_id: target.attachment_id + 1,
    ...
    f.sender
        ...
        .record_reinjection_frame_instance_with_suppression_interval(
            retired,
            &data_frame(f.remotes.stream_id(), 1024, 512),
            Duration::ZERO,
        );
    let ready = f
        .target_receivers
    ...
    let current = f
        ...
        .unwrap();
    assert_eq!(
        reliable_stream_frame_extent(&current.frame),
        Some((0, 1024, 1024))
    );
}
...
async fn mixed_remote_set() -> (
    ...
    let udp = remotes
        ...
        .instance();
    (context, remotes, _remote_input, tcp, udp)
}
...
async fn capacity_reference_is_the_fastest_mature_attached_path() {
    let (context, remotes, _remote_input, tcp, udp) = mixed_remote_set().await;
...
async fn expired_request_product_epoch_resets_successor_boundary_only_once() {
    let (context, remotes, _remote_input, _tcp, udp) = mixed_remote_set().await;
    context.install_relay_path_instance_for_test(udp);
...
async fn request_final_product_quantum_tags_only_the_remaining_floor_byte() {
    let (context, remotes, _remote_input, tcp, _udp) = mixed_remote_set().await;
    let stream_id = StreamId(172);
...
async fn request_detach_scrubs_qualification_without_deleting_product_debt() {
    let (context, mut remotes, _remote_input, owner, _survivor) = mixed_remote_set().await;
    let stream_id = StreamId(175);
    let payload_bytes = 4096;
    let frame = data_frame(stream_id, 0, payload_bytes);
    let mut controller = RequestMultipathController::new(stream_id);
...
async fn accepted_request_reinjection_removes_only_overlapping_tag_weight() {
    let (_context, _remotes, _remote_input, owner, repair) = mixed_remote_set().await;
    let stream_id = StreamId(174);
    let floor = reliable_path_startup_sample_limit_bytes(MuxLimits::default());
    let mut controller = RequestMultipathController::new(stream_id);
    controller.record_original_frame_for_test(
...
async fn request_load_becomes_idle_only_after_final_unique_original_ack() {
    let (context, remotes, _remote_input, owner, repair) = mixed_remote_set().await;
    let stream_id = StreamId(17);
