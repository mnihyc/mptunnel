...
#[test]
fn live_response_copy_coverage_tracks_ack_expiry_and_exact_membership() {
    let (binding, original, _original_receivers) = binding_for_underlay(UnderlayProtocol::Tcp);
    let copy = key(UnderlayProtocol::Tcp, 1);
    let (commands, _copy_receivers) = reliable_path_command_channels(8);
    binding.attach(
        copy.underlay,
        copy.path_id,
        commands,
    ...
    let deadline = binding.reinjection_suppression_deadline(&frame).unwrap();
    let before_expiry = deadline.checked_sub(Duration::from_nanos(1)).unwrap();
    assert_eq!(
        binding.live_reinjected_ranges_at(before_expiry),
        vec![range(0, 4096)]
    );

    binding.release_normalized_acked_ranges(&[range(1024, 2048)]);
    assert_eq!(
        binding.live_reinjected_ranges_at(before_expiry),
        vec![range(0, 1024), range(2048, 4096)],
        "positive receipt clips copy coverage without widening through the ACK hole",
    );
    assert!(binding.live_reinjected_ranges_at(deadline).is_empty());
    assert_eq!(
        binding.accepted_reinjected_data_in_flight_bytes_at(identity),
        3072,
        "expiry removes suppression, not unacknowledged exact-slot debt"
    );
    ...
    let instance = binding
        ...
        .path_instance_id;
    binding
        .begin_path_detach(copy, instance)
        .expect("withdraw exact copy publication");
    assert!(binding.live_reinjected_ranges_at(before_expiry).is_empty());
    let (replacement_commands, _replacement_receivers) = reliable_path_command_channels(8);
    binding.attach(
        copy.underlay,
        copy.path_id,
    ...
    assert_ne!(
        server_output_identity(&binding, copy).incarnation,
        identity.incarnation
    );
    assert!(
        binding.live_reinjected_ranges_at(before_expiry).is_empty(),
        "a same-key successor cannot revive its predecessor's still-unexpired copy"
    );
    assert!(
        binding
            .flights
            .lock()
            .expect("retained flight ledger")
            .values()
            .flat_map(|flights| flights.iter())
            .filter(|flight| flight.kind == CarrierWorkKind::ReinjectedData)
            .all(|flight| flight.reinjection_suppression_deadline == Some(deadline)),
        "coverage observation and membership removal do not rewrite retained D"
    );
}

#[test]
fn live_owner_recovery_gate_keeps_an_eligible_alternate_of_an_ineligible_owner() {
    let (binding, original, _original_receivers) = binding_for_underlay(UnderlayProtocol::Udp);
    binding.record_original_flight(original, &stream_data_frame_at(0, 4096));
    assert!(!binding.has_live_owner_recovery_alternative());
    let alternate = key(UnderlayProtocol::Tcp, 1);
    let (commands, _alternate_receivers) = reliable_path_command_channels(8);
    binding.attach(
        alternate.underlay,
        alternate.path_id,
    ...
    let owner = server_output_identity(&binding, original);
    assert!(binding.has_nonstale_reinjection_alternative(owner, TrafficClass::Throughput));
    assert!(binding.mark_output_stale(owner, TrafficClass::Throughput));
    assert!(binding.output_is_stale(owner));
    assert!(
        binding.has_live_owner_recovery_alternative(),
        "the coarse gate must not demand two payload-eligible outputs: only the distinct target needs service"
    );
    commands.begin_path_drain();
    assert!(
        !binding.has_live_owner_recovery_alternative(),
        "two retained entries cannot supply recovery when neither admits payload"
    );
}
