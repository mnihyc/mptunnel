...
fn assert_release_flight_equal(actual: &CarrierPathFlight, expected: &CarrierPathFlight) {
    assert_eq!(actual.key, expected.key);
    assert_eq!(actual.output_incarnation, expected.output_incarnation);
    assert_eq!(actual.configured_slot, expected.configured_slot);
    assert_eq!(actual.end, expected.end);
    assert_eq!(actual.bytes, expected.bytes);
    assert_eq!(actual.sent_at, expected.sent_at);
    assert_eq!(actual.kind, expected.kind);
    assert_eq!(
        actual.owner_fallback_deadline,
        expected.owner_fallback_deadline
    );
    assert_eq!(actual.assignment_range, expected.assignment_range);
    assert_eq!(
        actual.original_recovery_timing,
        expected.original_recovery_timing
    );
    assert_eq!(actual.evidence_eligible, expected.evidence_eligible);
    assert_eq!(actual.qualification_receipt, expected.qualification_receipt);
    assert_eq!(
        actual.reinjection_suppression_deadline,
        expected.reinjection_suppression_deadline
    );
}

fn assert_release_algorithms_equal(
    actual: &BTreeMap<u64, Vec<CarrierPathFlight>>,
    expected: &BTreeMap<u64, Vec<CarrierPathFlight>>,
    released: &[(u64, CarrierPathReleasedFlight)],
    reference: &[(u64, CarrierPathReleasedFlight)],
) {
    assert_eq!(actual.len(), expected.len());
    for ((start, entries), (expected_start, expected_entries)) in actual.iter().zip(expected) {
        assert_eq!(start, expected_start);
        assert_eq!(entries.len(), expected_entries.len());
        for (entry, expected_entry) in entries.iter().zip(expected_entries) {
            assert_release_flight_equal(entry, expected_entry);
        }
    }
    assert_eq!(released.len(), reference.len());
    for ((start, release), (expected_start, expected_release)) in released.iter().zip(reference) {
        assert_eq!(start, expected_start);
        assert_release_flight_equal(&release.flight, &expected_release.flight);
        assert_eq!(release.path_proving, expected_release.path_proving);
        assert_eq!(
            release.qualification_ambiguous_ranges,
            expected_release.qualification_ambiguous_ranges
        );
    }
}

#[test]
fn ack_intersection_release_preserves_metadata_order_and_all_small_ack_patterns() {
    use crate::model::product_qualification::ProductQualificationLedger;
    let sent_at = Instant::now();
    let mut entries = BTreeMap::<u64, Vec<CarrierPathFlight>>::new();
    // Same-key short members, partial copies, and untouched collision keys
    // exercise both Vec ordering and multiple earlier-origin retained fragments.
    for (id, start, end, kind) in [
        (1, 0, 12, CarrierWorkKind::OriginalData),
        (2, 0, 2, CarrierWorkKind::ReinjectedData),
        (3, 2, 12, CarrierWorkKind::ReinjectedData),
        (4, 2, 3, CarrierWorkKind::OriginalData),
        (5, 4, 12, CarrierWorkKind::OriginalData),
        (6, 8, 12, CarrierWorkKind::OriginalData),
    ] {
        let mut record = flight(
            key(UnderlayProtocol::Tcp, id),
            end,
            (end - start) as usize,
            kind,
        );
        record.sent_at = sent_at + Duration::from_micros(u64::from(id));
        record.output_incarnation = u64::from(id);
        record.configured_slot = Some(ConfiguredMemberSlot(id));
        record.assignment_range = range(0, 16);
        record.evidence_eligible = id != 4;
        if kind.is_original_transmission() {
            let mut qualification = ProductQualificationLedger::default();
            record.qualification_receipt = qualification
                .tag_admitted_original(end - start, end - start, range(start, end))
                .unwrap();
            let fallback_at = record.sent_at + Duration::from_micros(2);
            record.owner_fallback_deadline = Some(fallback_at);
            record.original_recovery_timing = Some(ReliableDataAckGapTiming {
                assignment_at: record.sent_at,
                loss_at: Some(record.sent_at + Duration::from_micros(1)),
                fallback_at,
            });
        } else {
            // Already-expired accepted copies still contribute to ACK ambiguity.
            record.reinjection_suppression_deadline = Some(sent_at);
        }
        entries.entry(start).or_default().push(record);
    }
    // Exhaustive byte coverage on the first eight bytes includes holes, adjacent
    // ACKs, empty coverage, all-prefix release, and exact endpoint collisions.
    for mask in 0..256_u16 {
        let ack = normalize_offset_ranges(
            (0..8)
                .filter(|byte| mask & (1 << byte) != 0)
                .map(|byte| range(byte, byte + 1))
                .collect(),
        );
        let mut actual = entries.clone();
        let mut expected = entries.clone();
        for ranges in [&ack[..], &ack[..], &[range(0, 16)][..]] {
            let released = release_carrier_path_flight_intersections(&mut actual, ranges);
            let reference = release_carrier_path_flight_ranges_full_scan(&mut expected, ranges);
            assert_release_algorithms_equal(&actual, &expected, &released, &reference);
        }
    }
    // Explicitly catch prepending each earlier-origin fragment separately,
    // which would reverse origins 0 and 2 at untouched key 4.
    let mut collision = entries;
    release_carrier_path_flight_intersections(&mut collision, &[range(3, 4)]);
    assert_eq!(
        collision[&4]
            .iter()
...
#[test]
fn ack_intersection_release_preserves_qualification_through_sack_and_replay() {
    use crate::model::product_qualification::ProductQualificationLedger;
    let mut qualification = ProductQualificationLedger::default();
    let mut reference_qualification = ProductQualificationLedger::default();
    let receipt = qualification
        .tag_admitted_original(16, 16, range(0, 16))
        .unwrap()
        .unwrap();
    assert_eq!(
    ...
    original.qualification_receipt = Some(receipt);
    let copy = flight(
        key(UnderlayProtocol::Udp, 1),
        8,
        4,
        CarrierWorkKind::ReinjectedData,
    );
    let mut actual = BTreeMap::from([(0, vec![original]), (4, vec![copy])]);
    let mut expected = actual.clone();
    // The actual qualification producer removes accepted-copy ambiguity first.
    assert_eq!(qualification.release_ambiguous(receipt, range(4, 8)), 4);
    assert_eq!(
        reference_qualification.release_ambiguous(receipt, range(4, 8)),
        4
    );
    for (ack, verified, outstanding) in [
        (vec![range(2, 6), range(10, 12)], 4, 8),
        (vec![range(2, 6), range(10, 12)], 4, 8),
        (vec![range(0, 16)], 12, 0),
    ] {
        let released = release_carrier_path_flight_intersections(&mut actual, &ack);
        let reference = release_carrier_path_flight_ranges_full_scan(&mut expected, &ack);
        assert_release_algorithms_equal(&actual, &expected, &released, &reference);
        for (ledger, releases) in [
            (&mut qualification, &released),
            (&mut reference_qualification, &reference),
        ] {
            for (start, release) in releases {
                if let Some(receipt) = release.flight.qualification_receipt {
                    let extent = range(*start, release.flight.end);
                    if release.path_proving {
                        ledger.release_exact(receipt, extent);
                    } else {
                        ledger.release_ambiguous(receipt, extent);
                    }
                }
            }
            let invariant = ledger.invariant();
            assert_eq!(invariant.verified_bytes, verified);
            assert_eq!(invariant.outstanding_tag_bytes, outstanding);
            assert!(invariant.holds());
        }
        assert_eq!(qualification, reference_qualification);
    }
}

#[test]
fn ack_intersection_release_leaves_large_unaffected_suffix_allocations_in_place() {
    let path = key(UnderlayProtocol::Tcp, 0);
    let mut actual =
        BTreeMap::from([(0, vec![flight(path, 16, 16, CarrierWorkKind::OriginalData)])]);
    // Test workload only: the candidate has no count or byte threshold. Spare
    // Vec capacity makes unnecessary rebuilds observable without wall-clock tests.
    for index in 1..=4096_u64 {
        let start = index * 32;
        let mut entries = Vec::with_capacity(3);
        entries.push(flight(path, start + 16, 16, CarrierWorkKind::OriginalData));
        actual.insert(start, entries);
    }
    let untouched = actual
        .range(32..)
        .map(|(&start, entries)| (start, entries.as_ptr(), entries.capacity()))
        .collect::<Vec<_>>();
    let mut expected = actual.clone();
    let ack = [range(0, 8)];
    let released = release_carrier_path_flight_intersections(&mut actual, &ack);
    let reference = release_carrier_path_flight_ranges_full_scan(&mut expected, &ack);
    assert_release_algorithms_equal(&actual, &expected, &released, &reference);
    for (start, pointer, capacity) in untouched {
        assert_eq!(
            actual[&start].as_ptr(),
            pointer,
            "untouched payload metadata moved at {start}"
        );
        assert_eq!(
            actual[&start].capacity(),
            capacity,
            "untouched Vec rebuilt at {start}"
        );
    }
}
