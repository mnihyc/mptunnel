...
impl H3SendHalf {
    fn cancel(&mut self) {
        match self {
            Self::Client(stream) => stream.stop_stream(h3::error::Code::H3_REQUEST_CANCELLED),
            Self::Server(stream) => stream.stop_stream(h3::error::Code::H3_REQUEST_CANCELLED),
        }
    }
}

/// Owns one HTTP/3 write while it is awaitable.
///
/// The h3 Quinn adapter retains a partially submitted DATA frame when its
/// future is cancelled. Retiring that request stream with the standard
/// H3_REQUEST_CANCELLED code keeps the partial frame and its failure local to
/// the request; unrelated MPP streams on the shared QUIC connection remain
/// valid.
struct H3SendOperation<'a> {
    owner: &'a mut H3SendStream,
    half: Option<H3SendHalf>,
}

impl<'a> H3SendOperation<'a> {
    fn begin(owner: &'a mut H3SendStream) -> Result<Self, QuicCarrierError> {
        if owner.finished {
    ...
    fn half_mut(&mut self) -> &mut H3SendHalf {
        self.half.as_mut().expect("active HTTP/3 send operation")
    }

    fn complete(mut self) {
        self.owner.inner = self.half.take();
    }
}

impl Drop for H3SendOperation<'_> {
    fn drop(&mut self) {
        let Some(mut half) = self.half.take() else {
            return;
        };
        half.cancel();
...
impl H3SendStream {
    ...
    pub(super) async fn finish(&mut self) -> Result<(), QuicCarrierError> {
        ...
        self.ensure_success_response().await?;
        let mut operation = H3SendOperation::begin(self)?;
        let finish_result = match operation.half_mut() {
    ...
    pub(super) async fn reject(&mut self) -> Result<(), QuicCarrierError> {
        ...
        if !matches!(self.inner, Some(H3SendHalf::Server(_))) {
            return Err(QuicCarrierError::H3Role(
                "only a pending server request can be rejected",
            ));
        }
        let mut operation = H3SendOperation::begin(self)?;
        let H3SendHalf::Server(stream) = operation.half_mut() else {
            unreachable!("server role checked before HTTP/3 rejection")
        };
    ...
    async fn ensure_success_response(&mut self) -> Result<(), QuicCarrierError> {
        ...
        let mut operation = H3SendOperation::begin(self)?;
        let H3SendHalf::Server(stream) = operation.half_mut() else {
            unreachable!("server role checked before HTTP/3 response")
        };
        stream
            .send_response(
                Response::builder()
