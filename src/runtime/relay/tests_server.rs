...
#[tokio::test]
async fn subsequent_bulk_request_ack_progresses_during_blocked_target_write() {
    subsequent_bulk_request_ack_progresses_during_blocked_target_delivery(false).await;
}

#[tokio::test]
async fn subsequent_bulk_request_ack_progresses_during_blocked_target_flush() {
    subsequent_bulk_request_ack_progresses_during_blocked_target_delivery(true).await;
}

async fn subsequent_bulk_request_ack_progresses_during_blocked_target_delivery(block_flush: bool) {
    const LATER_BYTES: usize = 1024;
    let limits = MuxLimits::default();
    let stream_id = StreamId(613 + u64::from(block_flush));
    let session_id = SessionId(stream_id.0);
    let lane = TrafficClass::Throughput;
    let first_bytes = usize::try_from(super::super::flow::reliable_flow_bulk_threshold_bytes(
        None, limits,
    ))
    .expect("bulk threshold fits usize");
    let total_bytes = first_bytes + LATER_BYTES;
    assert!(first_bytes <= reliable_relay_buffer_len(limits));
    assert!(
        (LATER_BYTES as u64)
            < crate::model::capacity::reliable_stream_ack_update_bytes(None, lane, limits)
    );
    let (commands, mut receivers) = reliable_path_command_channels(32);
    let binding =
        ResponseStreamBinding::new(session_id, UnderlayProtocol::Tcp, PathId(0), commands, lane);
    ...
    let feedback_interval = reliable_stream_recv_progress_interval(
        path_stream.send_path_snapshot(lane, crate::model::capacity::PATH_OPEN_SCORE_BYTES),
    );
    let initial_window =
        reliable_stream_initial_advertised_window_bytes(UnderlayProtocol::Tcp, lane, limits);
    let outbound_id = crate::product::OutboundId::parse("test-direct").expect("outbound ID");
    ...
    // First delivery leaves one byte of real target capacity. The next direct
    // write accepts one byte then parks; BufWriter accepts the whole small
    // write but its flush parks on that same target. Neither has an external
    // reader until the ACK/no-premature-credit assertions have been observed.
    let (mut application, local) = tokio::io::duplex(first_bytes + 1);
    let send_buffer = crate::runtime::stream::SessionSendBuffer::from_limits(limits);
    let relay = if block_flush {
        tokio::spawn(async move {
            relay_reliable_stream(
                tokio::io::BufWriter::with_capacity(total_bytes + 1, local),
                path_stream,
                &context,
                session_id,
                send_buffer,
        ...
        tokio::spawn(async move {
            relay_reliable_stream(local, path_stream, &context, session_id, send_buffer).await
        })
    };
    ...
    frames_tx
        ...
        .await
        .expect("first actual bulk request receipt");
    let mut observation = ServerBlockedDeliveryFeedback::default();
    tokio::time::timeout(
        Duration::from_secs(1),
        observation.until(
            &mut receivers,
    ...
    .await
    .expect("first ACK and exact post-consumption MAX published");
    assert!(
        first_receipt_started.elapsed() < feedback_interval,
        "second request must arrive before the first generation is already timer-due"
    );
    frames_tx
        .send(Ok(Frame::StreamData {
            stream_id,
        ...
        .await
        .expect("later contiguous subquantum bulk request receipt");
    let ack_while_blocked = tokio::time::timeout(
        Duration::from_secs(1),
        observation.until(&mut receivers, stream_id, total_bytes as u64, None),
    )
    .await
    .is_ok();
    while let Some(command) = try_recv_reliable_path_command(&mut receivers) {
        let pending_bytes = reliable_path_command_pending_bytes(&command);
        observation.observe(command, stream_id);
        receivers.release_pending_command_bytes(pending_bytes);
    }
    let max_while_blocked = observation.max_offset;
    let mut delivered = vec![0; total_bytes];
    tokio::time::timeout(
        Duration::from_secs(1),
        application.read_exact(&mut delivered),
    )
    .await
    .expect("release the actual target backpressure")
    .expect("target bytes");
    tokio::time::timeout(
        Duration::from_secs(1),
        observation.until(
            ...
            stream_id,
            total_bytes as u64,
            Some(initial_window + total_bytes as u64),
        ),
    )
    .await
    .expect("same target write/flush succeeds and publishes exact MAX");
    relay.abort();
    let _ = relay.await;
    let mut trailing = Vec::new();
    tokio::time::timeout(
        Duration::from_secs(1),
        application.read_to_end(&mut trailing),
    )
    .await
    .expect("cancelled relay releases its target writer")
    .expect("target reaches EOF");
    assert!(
        trailing.is_empty(),
        "a retained write must not append duplicate bytes"
    );
    assert_eq!(
        delivered,
        [first.as_ref(), later.as_ref()].concat(),
        "the retained partial write/flush must deliver each byte once"
    );
    assert_eq!(
        max_while_blocked,
        initial_window + first_bytes as u64,
        "ACK service while Pending must not publish new consumption credit"
    );
    assert!(
        ack_while_blocked,
        "later bulk request ACK must not await another DATA or target write/flush completion (block_flush={block_flush})"
    );
}

#[derive(Default)]
struct ServerBlockedDeliveryFeedback {
    ack_frontier: u64,
    max_offset: u64,
}

impl ServerBlockedDeliveryFeedback {
    fn observe(&mut self, command: ReliablePathCommand, stream_id: StreamId) {
        match command {
            ReliablePathCommand::SendFrame(Frame::StreamAck {
                ...
                self.ack_frontier = self
                    .ack_frontier
                    .max(stream_ack_contiguous_frontier(&ranges));
            }
            ReliablePathCommand::SendFrame(Frame::StreamMaxData {
                stream_id: id,
                max_offset,
    ...
    async fn until(
        ...
        while self.ack_frontier < ack_frontier
            || max_offset.is_some_and(|max| self.max_offset < max)
        {
            let command = recv_reliable_path_command(receivers)
                .await
                .expect("live server feedback output");
            let pending_bytes = reliable_path_command_pending_bytes(&command);
            self.observe(command, stream_id);
            receivers.release_pending_command_bytes(pending_bytes);
        }
    }
}
