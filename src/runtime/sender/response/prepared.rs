...
pub(in crate::runtime) fn claim_prepared_response_data(
    ...
    if let Err(error) = commitments.check_selected(identity, state.send_stream.data_ack_frontier())
    {
    ...
    if let Some(candidate) = recovery.candidate {
        ...
        let commit = |shape| {
            ...
            if let Err(error) =
                commitments.check_selected(identity, state.send_stream.data_ack_frontier())
            {
