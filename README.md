...
When a client is launched with both `tcp://` and `udp://` endpoint paths, mixed underlay is treated as its own Auto track. The same process can use TCP-underlay reliable streams, QUIC UDP reliable streams, and UDP datagram flows together for browsing, SSH-like streams, video/game-like UDP, downloads, and failover. Auto may move an ordered reliable stream across TCP and QUIC UDP paths only from live or configured ETA evidence; after that promotion, ordinary repair stays with the current carrier subflow set until that subflow set fails or is exhausted. This avoids blind same-stream TCP+UDP striping while still allowing mixed-carrier recovery and future measured cooperative aggregation without adding a manual transmission mode.
...
Datagrams are acknowledged with internal `DGRAM_FEEDBACK` ranges; client-side ACK observations update path RTT, jitter, loss, and delivery-rate inputs for later scheduling, while response datagrams are acknowledged back to the server. When several carriers are configured, one local datagram association can lazily own TCP and QUIC UDP carrier associations, but realtime datagrams prefer the ready lowest-ETA carrier instead of spraying ordinary probes across high-RTT paths. It opens survivor carriers for retry and failure recovery, uses a BBR-style runtime model from observed delivery rate, RTT, jitter, and loss to pace datagram sends and set response timeouts, and retries a timed-out datagram on a survivor carrier. Request blackholes mark the path failed; ACKed request/response-loss timeouts record packet loss without declaring the whole path dead. Session setup still uses scheduler inputs, adaptive health records, datagram freshness TTL, observed delivery rate, and active association load, then retries after path-level failures. Closed associations feed measured datagram delivery rate after enough useful payload has been observed and release their scheduler load. The same bounded authenticated idle-path probe loop exercises carrier handshakes and `PING`/`PONG` without opening datagram flows or adding probe traffic to active associations.
