...
use crate::runtime::sender_optional_reinjection_startup_floor_bytes;
...
#[tokio::test]
async fn server_data_ack_is_offered_before_first_target_write_completes() {
    use std::pin::Pin;
    ...
    let session_id = SessionId(717);
    let stream_id = StreamId(717);
    ...
    // Use the same initial grant as a successful target open. No receipt is
    // published by the fixture: only the actual DATA actor may create one.
    let initial_grant =
    ...
    let initial = try_recv_reliable_path_command(&mut receivers).expect("initial target grant");
    receivers.release_pending_command_bytes(reliable_path_command_pending_bytes(&initial));
    assert!(
        matches!(initial, ReliablePathCommand::SendFrame(Frame::StreamMaxData {
        stream_id: id, max_offset,
    }) if id == stream_id && max_offset == initial_grant)
    );
    assert!(try_recv_reliable_path_command(&mut receivers).is_none());
    assert_eq!(binding.feedback_status().ack_generation, 0);
    ...
    assert!(payload.len() as u64 <= initial_grant);
    frames_tx
        ...
        .await
        .expect("first legal DATA through the actor input");
    ...
    tokio::time::timeout(Duration::from_secs(1), async {
        while !pending.load(Ordering::Acquire) {
            tokio::select! {
                result = relay.as_mut() => panic!("live target must retain the actor: {result:?}"),
                () = tokio::task::yield_now() => {}
            }
        }
    })
    .await
    .expect("actual target write reaches backpressure");
    ...
    while let Some(command) = try_recv_reliable_path_command(&mut receivers) {
        ...
        if let ReliablePathCommand::SendFrame(frame @ Frame::StreamAck { .. }) = command {
            acknowledgements.push(frame);
        }
    }
    // The first receipt is urgent under the existing policy. This is the
    // offer-before-park boundary, not a byte threshold or ACK-cadence test.
    assert_eq!(
        acknowledgements,
        vec![Frame::StreamAck {
            stream_id,
            scope_start: None,
            ranges: vec![OffsetRange { start: 0, end: 2 }],
        }],
        "new DATA receipt must be offered before its target write completes",
    );
    assert_eq!(binding.feedback_status().ack_generation, 1);
    assert_eq!(
        binding.feedback_status().max_data.published_offset,
        Some(initial_grant),
        "a blocked receipt ACK must not grant the whole undelivered batch",
    );

    let mut delivered = [0_u8; 2];
    tokio::time::timeout(Duration::from_secs(1), async {
        tokio::select! {
            result = relay.as_mut() => panic!("open stream must remain live: {result:?}"),
            result = application.read_exact(&mut delivered) => result.expect("release retained write"),
        }
    })
    .await
    .expect("the same retained write completes when the target drains");
    assert_eq!(&delivered, payload.as_ref());
    assert_eq!(accepted.load(Ordering::Acquire), 2);

    // The real postwrite grant is a barrier: the first transaction, including
    // its postwrite feedback call, must finish before the next DATA arrives.
    tokio::time::timeout(Duration::from_secs(1), async {
        while binding.feedback_status().max_data.published_offset != Some(initial_grant + 2) {
            tokio::select! {
                result = relay.as_mut() => panic!("open stream must remain live: {result:?}"),
                () = tokio::task::yield_now() => {}
            }
    ...
    .await
    .expect("first write reaches its ordinary postwrite grant");
    let grant = try_recv_reliable_path_command(&mut receivers).expect("postwrite grant");
    receivers.release_pending_command_bytes(reliable_path_command_pending_bytes(&grant));
    assert!(
        matches!(grant, ReliablePathCommand::SendFrame(Frame::StreamMaxData {
        stream_id: id, max_offset,
    }) if id == stream_id && max_offset == initial_grant + 2)
    );
    assert!(try_recv_reliable_path_command(&mut receivers).is_none());
    assert_eq!(binding.feedback_status().ack_generation, 1);

    // The second two-byte receipt is no longer a first-ACK case. Also occupy
    // every ordinary control slot so its new generation must retain and wake
    // from real carrier capacity while the SAME target write remains blocked.
    for nonce in 0..8 {
        commands
            .try_enqueue_admitted_frame(Frame::Ping { nonce }, TrafficClass::Control)
            .expect("fill the existing eight-slot control queue");
    }
    ...
    frames_tx
        ...
        .await
        .expect("established subquantum DATA through the actor input");
    tokio::time::timeout(Duration::from_secs(1), async {
        while !pending.load(Ordering::Acquire) {
            tokio::select! {
                result = relay.as_mut() => panic!("live target must retain the actor: {result:?}"),
    ...
    .await
    .expect("second real target write reaches backpressure");
    assert_eq!(accepted.load(Ordering::Acquire), 3);
    let blocked = binding.feedback_status();
    assert_eq!(
        blocked.ack_generation, 2,
        "established receipt is materialized before parking"
    );
    assert!(blocked.ack.pending && !blocked.ack.published);
    assert_eq!(blocked.max_data.published_offset, Some(initial_grant + 2));
    for nonce in 0..8 {
        let filler = try_recv_reliable_path_command(&mut receivers).expect("held control slot");
        receivers.release_pending_command_bytes(reliable_path_command_pending_bytes(&filler));
        assert!(
            matches!(filler, ReliablePathCommand::SendFrame(Frame::Ping { nonce: actual })
            if actual == nonce)
        );
    }
    assert!(try_recv_reliable_path_command(&mut receivers).is_none());
    tokio::time::timeout(Duration::from_secs(1), async {
        while !binding.feedback_status().ack.published {
            tokio::select! {
                result = relay.as_mut() => panic!("target is still blocked: {result:?}"),
                () = tokio::task::yield_now() => {}
            }
        }
    })
    .await
    .expect("carrier capacity alone retries the newly pending generation");
    let ack = try_recv_reliable_path_command(&mut receivers).expect("retried second receipt");
    ...
    }) if id == stream_id && ranges == vec![OffsetRange { start: 0, end: 4 }])
    );
    assert!(try_recv_reliable_path_command(&mut receivers).is_none());
    assert_eq!(binding.feedback_status().ack_generation, 2);
    assert_eq!(
        binding.feedback_status().max_data.published_offset,
        Some(initial_grant + 2)
    );
    assert_eq!(
        accepted.load(Ordering::Acquire),
        3,
        "ACK retry did not drain the target"
    );
    ...
    tokio::time::timeout(Duration::from_secs(1), async {
        tokio::select! {
            result = relay.as_mut() => panic!("open stream must remain live: {result:?}"),
            result = application.read_exact(&mut second) => result.expect("release second write"),
        }
    })
    .await
    ...
    assert!(
        extra.is_empty(),
        "partial target writes must not be replayed"
    );
}
...
#[test]
fn live_owner_wake_retains_cause_and_successor_epoch_without_losing_due_state() {
    ...
    assert_eq!(
        server_live_owner_recovery_wake(Some(cause_deadline), Some(epoch_deadline), observed_at,),
        LiveOwnerRecoveryWake {
            due: false,
            deadline: Some(cause_deadline.into_std()),
        },
        "the retained cause clock remains the first structural recovery wake",
    );
    assert_eq!(
        ...
        "a due structural cause remains visible while the immutable successor epoch stays armed",
    );

    let due_at = observed_at + Duration::from_millis(30);
    assert_eq!(
        server_live_owner_recovery_wake(Some(cause_deadline), Some(epoch_deadline), due_at),
        LiveOwnerRecoveryWake {
            due: true,
            deadline: None,
        },
    ...
    assert_eq!(
        server_live_owner_recovery_wake(None, Some(epoch_deadline), due_at),
        LiveOwnerRecoveryWake {
            due: false,
            deadline: None,
        },
...
#[test]
fn live_owner_cause_does_not_wait_for_the_frontier_floor_epoch() {
    let observed_at = Instant::now();
    let due_cause = tokio::time::Instant::from_std(observed_at - Duration::from_millis(1));
    let epoch_deadline = observed_at + Duration::from_millis(20);

    assert_eq!(
        server_live_owner_recovery_wake(Some(due_cause), Some(epoch_deadline), observed_at),
        LiveOwnerRecoveryWake {
            due: true,
            deadline: Some(epoch_deadline),
        },
        "structural recovery remains actionable while the successor floor epoch is closed",
    );
}
...
fn ambiguous_prefix_ack_cannot_withdraw_a_fresh_response_tail_beyond_the_horizon() {
    ...
    let candidates = path_stream.data_ack_recovery_candidates(&[], TrafficClass::Throughput);
    assert!(
        candidates.is_empty(),
        "a positively acknowledged prefix does not prove that its fresh tail is missing",
    );
    ...
    let later_horizon_candidates = path_stream.data_ack_recovery_candidates(
        &[OffsetRange {
            start: horizon,
            end: tail_end,
        }],
    ...
    assert_eq!(
        ...
        quic_identity,
        "the same retained tail is eligible only when explicit gap evidence covers it",
    );
}

#[test]
fn reliable_recv_progress_sends_scoped_updates_for_both_carriers_without_delay() {
    ...
    for path in [tcp, udp] {
        ...
        assert_eq!(recv_stream.take_ack_update().len(), 1);
        ...
        for offset in [8192, 32768, 16384, 12288] {
            ...
            frames = recv_stream.take_ack_update();
        }
        ...
        assert_eq!(
            *scope_start, None,
            "a fill below the existing high-water adds no omission"
        );
        assert_eq!(ranges, &[OffsetRange::new(12288, 13312).unwrap()]);
    }
}
...
#[test]
fn positive_ack_updates_after_a_snapshot_do_not_add_negative_authority() {
    let limits = MuxLimits::default();
    let mut send_stream =
    ...
    let complete_ack =
        validate_stream_ack(Some(0), complete_prefix.to_vec(), send_stream.next_offset())
    ...
    update_reinjection_authoritative_ack_snapshot(&mut authoritative, &complete_ack, &send_stream);
    ...
    assert!(authoritative.gaps().is_empty());
...
#[test]
fn scoped_later_gap_does_not_classify_the_unknown_contiguous_frontier() {
    let mut send_stream = ReliableSendStream::new(StreamId(313), MuxLimits::default());
    send_stream.send_data(Bytes::from(vec![0x31; 300])).unwrap();
    ...
    .unwrap();
    send_stream.apply_validated_ack(&ack).unwrap();
    ...
    assert_eq!(
        server_data_ack_frontier_state(&evidence, &send_stream),
        ReliableDataAckFrontierState::Live
    );

    let prefix =
        begin_reliable_stream_ack(&send_stream, None, vec![OffsetRange { start: 0, end: 100 }])
            .unwrap();
    send_stream.apply_validated_ack(&prefix).unwrap();
    update_reinjection_authoritative_ack_snapshot(&mut evidence, &prefix, &send_stream);
    assert_eq!(send_stream.data_ack_frontier(), 100);
    assert_eq!(
        server_data_ack_frontier_state(&evidence, &send_stream),
...
fn response_retained_frontier_after_ack_history(
    ...
    for index in [0, 1, 3] {
        ...
        for frame in frames {
            ...
            assert!(scope_start.is_none() || index == 3);
            ...
            if authoritative.subsumes(&ack, &send_stream) {
                continue;
            }
            ...
            update_reinjection_authoritative_ack_snapshot(&mut authoritative, &ack, &send_stream);
        }
    }

    let expected_gaps = if aligned_snapshot.is_some() {
        vec![OffsetRange {
            start: (2 * quantum) as u64,
            end: (3 * quantum) as u64,
        }]
    } else {
        vec![]
    };
    ...
    assert_eq!(authoritative.gaps(), expected_gaps);
...
fn unknown_original_tail_reinjection_uses_remaining_output_after_persistent_stall() {
    ...
    send_stream
        .commit_prepared_data(&frame)
        .expect("commit original-transmission path data");
    // A positive prefix alone cannot prove the assigned tail missing. Anchor
    // the actual omission with a later receipt; the old test wrapper silently
    // substituted next_offset for a receiver-proven horizon.
    let ack = validate_stream_ack(
        Some(0),
        vec![
            OffsetRange {
    ...
    .expect("independently scoped unknown-owner gap");
    send_stream.apply_validated_ack(&ack).unwrap();
    let mut evidence = AuthoritativeStreamAckSnapshot::default();
    update_reinjection_authoritative_ack_snapshot(&mut evidence, &ack, &send_stream);
    assert_eq!(
    ...
    response_sender.record_delivered_data(1025);
...
async fn unknown_original_tail_reinjection_dispatches_as_path_failure_reinjection() {
    ...
    .expect("receiver-proven gap, not an invented assignment horizon");
    send_stream.apply_validated_ack(&ack).unwrap();
    let mut evidence = AuthoritativeStreamAckSnapshot::default();
    update_reinjection_authoritative_ack_snapshot(&mut evidence, &ack, &send_stream);
    assert_eq!(
...
#[tokio::test]
async fn live_owner_final_tail_does_not_become_path_failure_when_alternate_lacks_capacity() {
    let limits = MuxLimits::default();
    ...
    assert_eq!(
        same_epoch.queued, 1,
        "a rejected native Apply leaves the stable Product slot vacant, so restored target capacity must admit the retained recovery cause",
    );
}
...
fn response_fin_keeps_its_exact_decide_target_across_metric_churn() {
    ...
    let successor_deadline = response_sender
        ...
        .expect("accepted response FIN records successor G");
    assert!(
...
#[test]
fn failed_original_tail_reinjection_uses_structural_failure_authority() {
    let limits = MuxLimits::default();
...
#[test]
fn authoritative_gap_recovery_uses_a_vacant_structural_target() {
    let limits = MuxLimits::default();
    ...
    assert_eq!(
        outcome.queued, 1,
        "a due authoritative gap with a vacant target slot admits its exact retained frontier",
    );
    assert!(!outcome.pending);
    assert_eq!(
        response_sender.bytes(),
        1024,
        "the generic gap clock remains bounded by its exact blocking frontier",
    );
    ...
    assert_eq!(
        switched_target.queued, 1,
        "a queued copy that never reached Apply owns no stable publication slot; a distinct vacant replacement remains eligible",
    );
    ...
    assert_eq!(
        ...
        "the already queued exact frontier prevents an overlapping second Product transaction",
    );
}

#[test]
fn response_ack_gap_planner_extent_is_percentage_invariant() {
    let limits = MuxLimits::default();
    ...
    let instance_for = |key| {
        targets
            .iter()
            .find(|target| target.observation.key == key)
            .map(|target| target.observation.path_instance_id)
            .expect("current response attachment")
    };
    let original_slot = binding
        .configured_slot_for_instance_for_test(instance_for(original_key))
        .expect("original configured member slot");
    let reinjection_slot = binding
        .configured_slot_for_instance_for_test(instance_for(reinjection_key))
        .expect("reinjection configured member slot");
    assert_ne!(
        original_slot, reinjection_slot,
        "the paired fixture requires one structurally vacant target slot",
    );
    ...
    let original = send_stream
        .prepare_data(Bytes::from(vec![0x46; 4096]))
        .expect("prepare one owner-uniform response range");
    send_stream
        .commit_prepared_data(&original)
        .expect("commit one owner-uniform response range");
    binding.record_original_flight(original_key, &original);
    let ack_ranges = [
        OffsetRange {
            start: 0,
            end: 1024,
    ...
    for percent in [0, default_percent, 200] {
        ...
        sender.record_live_owner_frontier_floor_attempt(shared_epoch, Duration::from_secs(60));
        ...
        observations.push((percent, outcome.queued, outcome.pending, sender.bytes()));
    }

    for (percent, queued, pending, queued_bytes) in observations {
        assert_eq!(
            queued, 1,
            "percentage {percent} must not alter ACK-gap recovery eligibility",
        );
        assert!(!pending);
        assert_eq!(
            queued_bytes, 1024,
            "percentage {percent} must not alter the exact owner-uniform ACK-gap extent",
        );
    }
}
...
fn persistent_response_ack_gap_commits_only_the_ranked_frontier_quantum() {
    ...
    let validated_ack = begin_reliable_stream_ack(&send_stream, Some(0), ack_ranges.to_vec())
    ...
    assert_eq!(
        ...
        "the already-accepted response frontier must not be charged as a new payload a second time",
    );
    assert!(
        early_observation
            .target
    ...
    assert_eq!(
        ...
        None,
        "a pre-fallback structural response race must not consume the post-fallback successor epoch",
    );
    ...
    let frontier_frame = send_stream
        .retransmission_frames_for_ranges(authoritative_ack.gaps(), scored_frontier_bytes)
        .into_iter()
        .next()
...
fn draining_response_owner_retains_one_distinct_ack_gap_recovery_target() {
    ...
    let validated_ack = begin_reliable_stream_ack(&send_stream, Some(0), ack_ranges)
...
// Diagnostic of current behavior; refusal assertions are not permanent policy.
// Shared real Product/registry producer; no native socket, QUIC, or H3 claim.
// One-byte duplex capacity is the smallest deterministic partial-write device:
// two source bytes produce an observed successful prefix followed by Pending.
// The existing one-second relay-fixture settlement guard is unchanged in meaning.
//
// Controlled event order after the first target write parks:
//   ACK(actual response Original), DATA("cd"), PathDetached(exact old owner).
...
async fn server_target_backpressure_services_ack_and_detach_before_target_delivery() {
    ...
    // Drive the real actor and inspect a synchronous producer observation after
    // its poll. Pending is never inferred from a sleep or an elapsed deadline.
    async fn drive_until<F, T>(mut relay: Pin<&mut F>, ready: impl Fn() -> bool)
    where
        F: Future<Output = T>,
        T: std::fmt::Debug,
    ...
    tokio::time::timeout(Duration::from_secs(1), async {
        ...
        let router = ClientIngressRouter::new(
            &ProductPolicyConfig {
                ...
                routes: vec![RouteRuleSpec::new(
                    ...
                    RouteMatchSpec::default(),
                    RouteAction::allow_restricted(
                        EgressAction::Outbound(id), None, InitialDemand::Automatic,
                    ),
                )],
            }, outbound_registry,
        ).expect("router");
        ...
        let (registry, mut accepted_rx) = ServerReliableStreamRegistry::new_accepting_with_limits(
            limits, ResourceLimits::default().max_paths,
        );
        let port = registry.path_port();
        let registration = port.register_test_carrier_path(
            session_id, UnderlayProtocol::Tcp, PathId(0), ServerLocalPathProperties::default(),
        );
        let key = CarrierPathKey { underlay: registration.underlay(), path_id: registration.path_id() };
        let path_instance = registration.path_instance_id();
        let target = TargetAddr::Ip("127.0.0.1:443".parse().expect("target identity"));
        let open = |commands, phase| ServerStreamOpenRequest {
            session_id,
            stream_id,
            target: target.clone(),
            initial_demand: StreamDemandHint::Throughput,
            return_plan: StreamReturnPlan { phase, ..Default::default() },
            attachment: ServerStreamPathAttachment {
                path_registration: registration.clone(), commands,
                max_frame_payload_bytes: limits.max_payload_bytes,
            },
            mux_limits: limits,
        };
        ...
        assert!(matches!(
            port.open_or_attach(open(commands, StreamAttachmentPhase::Create)).await.unwrap(),
            ServerStreamOpenOutcome::New(_),
        ));
        let zero = recv_reliable_path_command(&mut receiver).await.expect("initial acceptance");
        receiver.release_pending_command_bytes(reliable_path_command_pending_bytes(&zero));
        assert!(matches!(zero, ReliablePathCommand::SendFrame(Frame::StreamMaxData {
            stream_id: id, max_offset: 0,
        }) if id == stream_id));
        ...
        let old = binding.sender_path_targets(lane, 1)[0].observation;
        assert_eq!(old.path_instance_id, path_instance);
        drop(accepted);

        // Supply the ordinary successful-target request grant, exactly as the
        // existing WriteObserved relay-body fixture does. Response credit is a
        // real routed peer frame, not a send-state mutation.
        ...
        port.route_frame(&registration, stream_id, Frame::StreamData {
            stream_id, offset: 0, payload: Bytes::from_static(b"ab"),
        }).await.expect("first request batch");
