...
For product datagrams, current diagnostics record path selection and successful
carrier writes through `datagram_path_selected`,
`tcp_datagram_request_written`, and `udp_datagram_request_written`, plus
server-side request/response milestones. They do not emit private
`tcp_datagram_response_timeout` or `udp_datagram_response_timeout` events.
The runtime still derives feedback and retry budgets from the carrier model and
remaining product TTL: an unattempted alternative gets one modeled
pre-feedback timeout, while the final attempt may use the larger bounded
budget. A same-family fallback must select a different configured path.
Matching feedback forbids replay. Attribute an external UDP timeout by
correlating the durable selection, write, and server milestones; absence of a
private response-timeout event is expected and does not prove that the
pre-feedback timer failed.
...
An optimization is accepted only when at least one intended owner is proven
better and no protected cell is proven worse; exact all-zero deterministic
equivalence may also pass. Report every run plus median, spread, and
preregistered two-sided 95% paired-bootstrap interval for receiver-delivered
goodput, latency/gaps, traffic overhead, CPU, memory, and path use together.
An interval wholly above zero proves normalized improvement, one wholly below
zero proves regression, and overlap with zero is `INCONCLUSIVE`. Never turn one
roughly five-percent movement into a hard threshold. Rerun inconclusive
evidence without persisting, promoting, or reverting it; correct or revert
only a proven repeated regression unless an explicit theoretical
latency/stability tradeoff is approved.
