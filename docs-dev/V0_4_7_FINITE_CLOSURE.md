...
| T03 | P4-score | Does action rank contain only `T + ceil(8(A+M)/C)`, with coherent local pre-native `A`, jitter confined to timing deadband `U`, and a canonical action/carrier/incarnation tie key? | Deterministic REDs for field mutation, divided capacity, encoded action work, rounding, invalid-rate fallback, overflow, Suspect, combined deadband, `expensive`/backup tiers, and permutation-invariant identity. | One advisory rank correction. It is not a delivery ETA or admission/deadline proof. If coherent `A` is unavailable across a comparison domain, omit it uniformly; never invent exact debt. |
