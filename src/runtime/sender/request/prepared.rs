...
pub(in crate::runtime) fn claim_prepared_request_data(
    ...
    if let Err(error) = commitments.check_selected(instance, state.send_stream.data_ack_frontier())
    {
    ...
    if let Some(candidate) = recovery.candidate {
        ...
        let result = candidate
            .commit_with_current_native_shape(&commands, |shape| {
                ...
                if let Err(error) =
                    commitments.check_selected(instance, state.send_stream.data_ack_frontier())
                {
    ...
    let ready_instances = commitments.original_ready(
        current_ready_instances(&state),
        state.send_stream.data_ack_frontier(),
        &mut commitment_waits,
    );
