...
pub(in crate::runtime::path::tcp) async fn handle_client_tcp_capacity_frame(
    frame: Frame,
    ...
    match frame {
        ...
        Frame::PathCapacityReceipt {
            ...
            let kernel_delivery_rate_bps = native_observation
                .and_then(super::super::metrics::TcpNativeObservation::delivery_rate_bps)
                .unwrap_or(0);
            #[cfg(feature = "lab-diagnostics")]
            let kernel_pacing_rate_bps = native_observation
                .and_then(super::super::metrics::TcpNativeObservation::pacing_rate_bps)
                .unwrap_or(0);
