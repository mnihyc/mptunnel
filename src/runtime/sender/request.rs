        ...
        loop {
            ...
            if let Some(prefix) = early_prefix {
                if prefix.end < scored.end {
                    // Rank the actual uncopied extent, not a larger range
                    // containing bytes whose completion opportunity is spent.
                    range.end = prefix.end;
                    continue;
                }
            } else if deadline > observed_at {
            ...
            if early_completion_copy {
                // Keep the normal owner clock/deadline above. Only this
                // separately bounded completion authority may use a current
                // startup-prior target, projected from the SAME Native inputs.
                model = self
                    .multipath
                    .prepared_completion_reinjection_model_from_observation(
                        context,
                        remotes,
                        preview,
                        lane,
    ...
    pub(super) fn commit_prepared_recovery(
        ...
        if candidate.early_completion_copy {
            ...
            let owner = self
                .multipath
                .live_owner_uniform_frontier(range, &remotes.path_instances());
            if candidate.lane != TrafficClass::Latency
                || !matches!(
                    candidate.cause,
                    RelaySendCause::CompletionTailReinjection(_)
                )
                || self.multipath.raw_uncopied_prefix(range) != Some(range)
                || owner.is_none_or(|frontier| {
                    frontier.range != range
                        || frontier.owners.len() != 1
                        || frontier.owners.contains(&target)
                })
            {
                return Err(RuntimeError::SenderServiceBlocked);
            }
