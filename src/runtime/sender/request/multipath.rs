...
impl RequestMultipathController {
    ...
    /// An already authorized early completion may rank a startup-prior target.
    /// This pure projection grants no loss authority, observes no new clock,
    /// and does not promote the target's prior into measured Product evidence.
    #[allow(clippy::too_many_arguments)]
    pub(super) fn prepared_completion_reinjection_model_from_observation(
        &self,
        context: &ClientPathContext,
        remotes: &ReliableRelayRemoteSet,
        preview: &Frame,
    ...
    #[allow(clippy::too_many_arguments)]
    fn recovery_reinjection_model_from_observation(
        &self,
        context: &ClientPathContext,
        remotes: &ReliableRelayRemoteSet,
        preview: &Frame,
        ...
        let reinjection_path = loop {
            let position = match self.choose_lowest_eta_relay_path_for_extent(
                ...
                Err(RequestMultipathPlanError::ServiceBlocked) => {
                    // The cause's target eligibility was present, but every
                    // exact alternate's native queue was full. Preserve that
                    // distinction so the actor waits
    ...
    pub(super) fn raw_uncopied_prefix(&self, range: OffsetRange) -> Option<OffsetRange> {
        self.request.flights.raw_uncopied_prefix(range)
    }
