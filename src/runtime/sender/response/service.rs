...
impl ServerResponseSenderService {
    ...
    fn prepared_recovery_for_range(
        ...
        loop {
            ...
            {
                range.end = prefix.end;
                continue;
            }
            let early_completion_copy = early_prefix.is_some();
            if early_completion_copy && proven {
                // This is bounded completion authority, not an assertion of
                // measured bulk service. Rank ordinary admissible startup
                // targets too, from the same captured observation.
                target = select_response_frame_path_for_extent(
                    &available,
                    lane,
                    preview,
                    CarrierEmitMode::Classified,
            ...
            let cause = if proven && !early_completion_copy {
