//! Versioned MPP wire vocabulary and bounded codecs.
//!
//! Only peer-owned facts belong here; ingress, outbound, and configured path
//! policy stay with their local endpoint owners.

pub mod auth;
pub mod codec;
pub(crate) mod frame;
pub(crate) mod path_capacity;
mod types;

pub use types::{
    AuthNonce, AuthTag, CloseReason, ConfiguredMemberSlot, DatagramFlowId, DatagramId, Frame,
    IpPacketId, IpTunnelId, NativeDeliverySnapshot, OffsetRange, PATH_METRIC_APPROXIMATE_LOSS,
    PATH_METRIC_APPROXIMATE_MASK, PATH_METRIC_APPROXIMATE_PACING, PATH_METRIC_APPROXIMATE_QUALITY,
    PATH_METRIC_APPROXIMATE_RATE, PATH_METRICS_MAX_RATE_VALID_FOR_US, PathId, PathMetricDirection,
    PathMetrics, PathUsage, PeerPathState, PeerPathStatus, PeerStatusCode, ResetReason, SessionId,
    StreamAttachmentPhase, StreamDemandHint, StreamId, StreamReturnPlan, TargetAddr,
    UnderlayProtocol,
};
