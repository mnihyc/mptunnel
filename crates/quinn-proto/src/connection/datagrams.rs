...
impl Datagrams<'_> {
    ...
    pub fn send(&mut self, data: Bytes, drop: bool) -> Result<(), SendDatagramError> {
        ...
        let Some(memory_after_push) = data.len().checked_add(size_of::<Datagram>()) else {
            return Err(SendDatagramError::TooLarge);
        };
        // The peer's frame-size limit applies to payload. The local send-buffer limit applies to
        // the complete queued entry, including the metadata accounted by `memory_used()`.
        if data.len() > max || memory_after_push > send_buffer_size {
            return Err(SendDatagramError::TooLarge);
        }
        if drop {
            self.conn
...
impl DatagramState {
    ...
    fn has_send_buffer_space(&self, datagram_len: usize, send_buffer_size: usize) -> bool {
        let Some(total) = self
            ...
            .and_then(|used| used.checked_add(datagram_len))
        else {
            return false;
        };
