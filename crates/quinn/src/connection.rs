...
impl Connection {
    ...
    /// Snapshot the exact active path and its congestion controller atomically.
    ///
    /// Unlike separate [`Self::stats`] and [`Self::congestion_state`] calls,
    /// the returned RTT, flight, application-limited state, and controller
    /// clone all come from one acquisition of the connection-state lock.
    pub fn active_path_snapshot(&self) -> ActivePathSnapshot {
        self.0
            .state
            .lock("active_path_snapshot")
            .inner
    ...
    ///
    /// The notification is serialized with packet processing and path changes
    /// by the connection-state lock.
    pub fn mark_application_ready(&self) {
        self.0
            .state
            .lock("mark_application_ready")
            .inner
            .mark_application_ready();
    }
