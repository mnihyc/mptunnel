...
use crate::model::work::{
    ...
    reliable_live_frontier_reinjection_limit_bytes, reliable_reinjection_service_limit_bytes,
};
use crate::mux::MuxLimits;
use crate::mux::stream::ReliableSendStream;
use crate::performance::MppPerformanceConfig;
use crate::protocol::frame::{offset_ranges_not_covered, reliable_stream_frame_accounted_bytes};
#[cfg(feature = "lab-diagnostics")]
...
/// One exact recovery candidate from caller-supplied Native observations.
/// The final query and normal queue Apply run under Native -> Product.
...
#[derive(Default)]
pub(in crate::runtime) struct ResponsePreparedRecoveryObservation {
    pub(in crate::runtime) candidate: Option<ResponsePreparedRecoveryCandidate>,
...
impl ServerResponseSenderService {
    ...
    pub(in crate::runtime) fn next_prepared_recovery(
        ...
        // Observation scope is the declared sequential probe's positive 64-byte
        // reply (including fragments), not a Product admission limit.
        macro_rules! trace_small_query {
            ($range:expr, $stage:literal, $fields:expr $(,)?) => {
                #[cfg(feature = "lab-diagnostics")]
                {
                    let trace_range = $range;
                    if trace_range.end > trace_range.start
                        && trace_range.end - trace_range.start <= 64
                        && lab_diagnostic_event_enabled("prepared_recovery_query")
                    {
                        let ready_targets = targets
                            ...
                            .count();
                        lab_diagnostic(
                            "prepared_recovery_query",
                            format_args!(
                                "session_id={} stream_id={} binding_identity={:p} offset={} end={} payload_bytes={} frontier={} sent_offset={} lane={:?} stage={} supplied_targets={} supplied_ready={} ready_targets={} event_lag_us={} {}",
                                self.session_id.0,
                                self.stream_id.0,
                                binding,
                                trace_range.start,
                                ...
                                ready.len(),
                                ready_targets,
                                Instant::now().saturating_duration_since(observed_at).as_micros(),
                                $fields,
                            ),
                        );
                    }
        ...
        let Some(mut range) = offset_ranges_not_covered(&retained, &covered)
            ...
            #[cfg(feature = "lab-diagnostics")]
            if let Some(range) = retained.first() {
                trace_small_query!(*range, "no_uncovered_range", format_args!(""));
            }
            return result;
        };
        for queued in self.queue.queued_reinjection_ranges() {
            if queued.start <= range.start && queued.end > range.start {
                trace_small_query!(range, "queued_overlap", format_args!(""));
                return result;
            }
        ...
        if selection_quantum == 0 {
            trace_small_query!(
                range,
                "selection_quantum_zero",
                format_args!("selection_quantum=0"),
            );
            return result;
        }
        range.end = range
        ...
        loop {
            let Some(frontier) = binding.live_owner_uniform_frontier(range) else {
                trace_small_query!(range, "no_uniform_frontier", format_args!(""));
                return result;
            };
            ...
            let Some((timing, boundary)) =
                binding.observe_prepared_recovery_timing(scored, |identity| {
                    targets
                        .iter()
                        .find(|target| {
                            target.observation.key == identity.key
                                && target.observation.incarnation == identity.incarnation
                        })
                        .map(response_completion_snapshot)
                })
            else {
                trace_small_query!(scored, "no_owner_timing", format_args!(""));
                return result;
            };
            ...
            trace_small_query!(
                ...
                "ranked",
                format_args!(
                    "proven={} owner_underlay={:?} owner_path_id={} owner_incarnation={} assignment_age_us={} loss_from_assignment_us={:?} fallback_from_assignment_us={} deadline_from_assignment_us={} due={} ready_service_targets={} nonavoided_service_targets={} target_underlay={:?} target_path_id={:?} target_instance={:?} target_incarnation={:?} target_completion_us={:?} owner_completion_us={:?} assignment_boundary={}",
                    proven,
                    owner.key.underlay,
                    owner.key.path_id.0,
                    owner.incarnation,
                    observed_at
                        .saturating_duration_since(timing.assignment_at)
                        .as_micros(),
                    timing.loss_at.map(|at| at
                        .saturating_duration_since(timing.assignment_at)
                        .as_micros()),
                    timing
                        .fallback_at
                    ...
                    target
                        .as_ref()
                        .map(|target| target.observation.key.underlay),
                    target
                        .as_ref()
                        .map(|target| target.observation.key.path_id.0),
                    target
                        .as_ref()
                        .map(|target| ResponseAcquisitionOutputId::from(target)
                            .path_instance_id
                            .as_u64()),
                    target.as_ref().map(|target| target.observation.incarnation),
                    completion.map(|duration| duration.as_micros()),
                    owner_completion.map(|duration| duration.as_micros()),
                    boundary,
                ),
            );
            if deadline > observed_at {
            ...
            let extent = reliable_live_frontier_reinjection_limit_bytes(
                adaptive_reliable_relay_reinjection_bytes(Some(snapshot), lane, mux_limits),
                selection_quantum,
                flight_interval_bytes(scored.start, scored.end),
                send_stream.reinjection_bytes(),
                mux_limits,
            ...
            if extent == 0 {
                trace_small_query!(
                    ...
                    "target_extent_zero",
                    format_args!(
                        "target_service={} selection_quantum={}",
                        target_service, selection_quantum
                    ),
                );
                return result;
            ...
            let Some(frames) = exact_contiguous_retransmission_frames(
                ...
                trace_small_query!(scored, "no_retained_apply_frame", format_args!(""));
                return result;
            };
            let cause = if proven {
                RelaySendCause::persistent_server_ack_gap_reinjection(identity, snapshot)
            } else {
    ...
    /// Exact normal repair queue Apply after the caller repeats the candidate
    /// query under the selected Native shape and final Product lock.
    pub(in crate::runtime) fn commit_prepared_recovery(
        &mut self,
        binding: &ResponseStreamBinding,
        send_stream: &ReliableSendStream,
        candidate: &ResponsePreparedRecoveryCandidate,
        ...
        #[cfg(feature = "lab-diagnostics")]
        let trace_apply = end > start
            && end - start <= 64
            && lab_diagnostic_event_enabled("prepared_recovery_apply");
        if self.queue.has_queued_reinjection_overlap(&candidate.frame)
            || binding
                ...
                .contains(&(candidate.target.key, candidate.target.incarnation))
        {
            ...
            if trace_apply {
                lab_diagnostic(
                    "prepared_recovery_apply",
                    format_args!(
                        "session_id={} stream_id={} binding_identity={:p} offset={} end={} payload_bytes={} frontier={} lane={:?} cause={} target_underlay={:?} target_path_id={} target_instance={} target_incarnation={} stage=prevalidation_refused accepted=false",
                        self.session_id.0,
                        self.stream_id.0,
                        binding,
                        start,
                        ...
                        send_stream.data_ack_frontier(),
                        candidate.lane,
                        candidate.cause.as_str(),
                        candidate.target.key.underlay,
                        candidate.target.key.path_id.0,
                        candidate.target.path_instance_id.as_u64(),
                        candidate.target.incarnation,
                    ),
                );
            }
            return Err(RuntimeError::SenderServiceBlocked);
        ...
        #[cfg(feature = "lab-diagnostics")]
        let accept_begin_unix_ms = trace_apply.then(|| {
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .expect("lab observation after Unix epoch")
                .as_millis()
        });
        let admission = binding.try_enqueue_prepared_recovery_with_shape(
            &candidate.target,
            &candidate.frame,
            candidate.lane,
            self.queue
        ...
        #[cfg(feature = "lab-diagnostics")]
        if let Some(accept_begin_unix_ms) = accept_begin_unix_ms {
            lab_diagnostic(
                "prepared_recovery_apply",
                format_args!(
                    "session_id={} stream_id={} binding_identity={:p} offset={} end={} payload_bytes={} frontier={} lane={:?} cause={} target_underlay={:?} target_path_id={} target_instance={} target_incarnation={} stage=queue_result accepted={} accept_begin_unix_ms={} copy_suppression_remaining_us={:?} admission_error={:?}",
                    self.session_id.0,
                    self.stream_id.0,
                    binding,
                    start,
                    ...
                    candidate.target.incarnation,
                    admission.is_ok(),
                    accept_begin_unix_ms,
                    admission.as_ref().ok().map(|deadline| deadline
                        .saturating_duration_since(Instant::now())
                        .as_micros()),
                    admission.as_ref().err(),
                ),
            );
        }
...
#[cfg(test)]
#[path = "tests_prepared_recovery_service.rs"]
