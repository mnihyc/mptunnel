...
impl RequestSenderService {
    ...
    async fn send_frame(
        ...
        let avoid_keys = match cause {
            ...
            cause if cause.is_repair() => self.request.flights.sent_keys_for_frame(&sent_frame),
            _ => Vec::new(),
        };
        let instance = self
    ...
    pub(in crate::runtime) fn enqueue_live_owner_tail_repair(
        ...
        for frame in repair_frames {
            ...
            let owner_keys = self.request.flights.live_owner_tail_repair_owner_keys(
                &frame,
                &live_instances,
                first_repair_after,
                repeat_repair_after,
    ...
    pub(in crate::runtime) fn enqueue_failed_path_gap_repairs(
        ...
        let ranges = self
            .request
            .flights
            .latest_unacked_ranges_for_path(failed_key);
        self.enqueue_failed_path_gap_repairs_for_ranges(
            sender_queue,
            context,
            remotes,
